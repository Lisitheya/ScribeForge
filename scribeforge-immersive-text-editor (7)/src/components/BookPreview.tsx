...
export default function BookPreview() {
  ...
  const getPdf = async () => {
    commitEdits();
    setBusy(true);
    try { await printToPdf({ ...doc, html: pages.join('') }, colors, cfg); }
    ...
    <div className="modal-veil" onMouseDown={e => { if (e.target === e.currentTarget) { commitEdits(); patchUi({ modal: null }); } }}>
      <div className="modal preview-modal">
        ...
        <div className="modal-foot">
          ...
          <button className="btn acc" onClick={getPdf} disabled={busy} style={{ minWidth: 160 }}>
            {busy ? <Loader2 size={14} className="spin" /> : <CloudDownload />}
            {busy ? 'Готовлю…' : 'Сохранить в PDF'}
          </button>
        </div>
      </div>
    </div>
