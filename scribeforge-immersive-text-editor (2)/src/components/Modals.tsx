...
import {
  ...
  Table2, Settings as SettingsIcon, Trash2, Archive, Upload,
} from 'lucide-react';
...
import {
  ...
  buildPrintHtml, printCss, notebookOrder, download,
...
export default function Modals({ onPrint }: { onPrint: (cfg: PrintConfig) => void }) {
  ...
  return (
    <div className="modal-veil" onMouseDown={e => { if (e.target === e.currentTarget) useStore.getState().patchUi({ modal: null }); }}>
      {modal === 'export' && <ExportModal onPrint={onPrint} />}
...
/* ═══ Экспорт / Книга ═══ */
function ExportModal({ onPrint }: { onPrint: (cfg: PrintConfig) => void }) {
  ...
  const quickPdf = async () => {
    ...
    try {
      await downloadPdf(doc, colors, cfg);
    } catch (e) {
      console.error(e);
      setPdfErr(true);
    }
      ...
      {tab === 'pdf' && (
        <div className="modal-foot">
          <span style={{ fontSize: 11, color: 'var(--sf-muted)', marginRight: 'auto' }}>
            {pdfErr
              ? 'Не удалось собрать PDF — попробуйте «Печать → Сохранить как PDF».'
              : '«Скачать PDF» — файл сразу. «Печать» — векторный PDF с точной разбивкой страниц.'}
          </span>
          <button className="btn" onClick={() => useStore.getState().patchUi({ modal: 'preview' })}><BookOpen /> Предпросмотр</button>
          <button className="btn" onClick={() => { onPrint(cfg); close(); }}><Printer /> Печать…</button>
          <button className="btn acc" onClick={quickPdf} disabled={pdfBusy} style={{ minWidth: 150 }}>
...
/* ═══ Печать (портал) ═══ */
export function PrintView({ doc, colors, cfg, onDone }: {
  doc: NonNullable<ReturnType<typeof useActiveDoc>>; colors: any; cfg: PrintConfig; onDone: () => void;
}) {
  ...
  return createPortal(
    <div id="print-root" ref={ref}>
      <style>{printCss(doc, colors, cfg)}</style>
      <div className="pv" data-skin={doc.skin}
        dangerouslySetInnerHTML={{ __html: buildPrintHtml(doc, colors, cfg)
