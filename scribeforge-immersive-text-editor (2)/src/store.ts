...
// запись в localStorage с дебаунсом — не тормозит набор текста на больших рукописях
const debouncedStorage = (() => {
  ...
  return {
    ...
    setItem: (k: string, v: string) => {
      pending = { k, v };
      clearTimeout(t);
      t = setTimeout(() => { try { if (pending) localStorage.setItem(pending.k, pending.v); } catch { /* quota */ } }, 800);
    },
