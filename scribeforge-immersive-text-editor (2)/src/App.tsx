...
import Modals, { PrintView } from './components/Modals';
import type { PrintConfig } from './utils/export';
  ...
  return (
    <div className={`sf-app skin-${skin} ${ui.focus ? 'focus' : ''} ${settings.reducedFx ? 'reduced-fx' : ''} ${settings.dyslexia ? 'dyslexia' : ''}`}>
      {ui.view === 'home' ? (
        <>
          <HomePage />
          <Modals onPrint={cfg => setPrint(cfg)} />
          {print && doc && colors && <PrintView doc={doc} colors={colors} cfg={print} onDone={() => setPrint(null)} />}
        </>
      ) : (
      <>
      <TopBar />
      ...
      <Modals onPrint={cfg => setPrint(cfg)} />
      {print && doc && colors && (
        <PrintView doc={doc} colors={colors} cfg={print} onDone={() => setPrint(null)} />
      )}
      </>
      )}
    </div>
